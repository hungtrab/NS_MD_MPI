...
echo "Running Hopper Multi (TRPO)"
echo "═══════════════════════════════════════"

# Count configs
CONFIGS=(configs/TRPO/multi/hopper_*_baseline_trpo.yaml)
NUM_CONFIGS=${#CONFIGS[@]}
...
# Baseline runs
echo "=== BASELINE ==="
for config in configs/TRPO/multi/hopper_*_baseline_trpo.yaml; do
    if [ -f "$config" ]; then
...
echo ""
echo "=== NS-MDMPI ===" 
for config in configs/TRPO/multi/hopper_*_nsmdmpi_trpo.yaml; do
    if [ -f "$config" ]; then
        filename=$(basename "$config" .yaml)
        echo "  Starting: $filename"
        conda run -n rl_hf_course python scripts/train.py \
