...
echo "Running Hopper Multi (SAC)"
echo "═══════════════════════════════════════"

# Count configs
CONFIGS=(configs/SAC/multi/hopper_*_baseline_sac.yaml)
NUM_CONFIGS=${#CONFIGS[@]}
...
# Baseline runs
echo "=== BASELINE ==="
for config in configs/SAC/multi/hopper_*_baseline_sac.yaml; do
    if [ -f "$config" ]; then
...
echo ""
echo "=== NS-MDMPI ===" 
for config in configs/SAC/multi/hopper_*_nsmdmpi_sac.yaml; do
    if [ -f "$config" ]; then
        filename=$(basename "$config" .yaml)
        echo "  Starting: $filename"
        conda run -n rl_hf_course python scripts/train.py \
