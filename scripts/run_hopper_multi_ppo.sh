...
echo "Running Hopper Multi (PPO)"
echo "═══════════════════════════════════════"

# Count configs
CONFIGS=(configs/PPO/multi/hopper_*_baseline_ppo.yaml)
NUM_CONFIGS=${#CONFIGS[@]}
...
# Baseline runs
echo "=== BASELINE ==="
for config in configs/PPO/multi/hopper_*_baseline_ppo.yaml; do
    if [ -f "$config" ]; then
...
echo ""
echo "=== NS-MDMPI ===" 
for config in configs/PPO/multi/hopper_*_nsmdmpi_ppo.yaml; do
    if [ -f "$config" ]; then
        filename=$(basename "$config" .yaml)
        echo "  Starting: $filename"
        conda run -n rl_hf_course python scripts/train.py \
